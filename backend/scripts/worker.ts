 ...
 *   npm run worker -- --once --live  # persist real sends (needs a real sender)
 *
 ...
 *
 * Dry-run is the default until Gmail sending is real: drafts are composed and
 * recorded as `draft_composed` events, no mail leaves the machine, and no
 * pursuit advances to `contacted`.
 */
