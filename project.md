...
Retrieval upgrade (2026-09-12): the optional OpenRouter agent consumes preserved listing URLs, reads their broker section before search, and runs one budgeted alternate discovery stage if no supported broker is found. Offline coverage includes the Fatma Kara layout, wrong-city results, co-broker retention, and spending limits; all 135 tests pass. A subsequent live search-recovery test cost 0.84¢ and still missed Fatma Kara; direct-page retrieval was not exercised because the original input lacked a URL. Unsupported candidates were flagged, but generic office values leaked into one candidate’s personal contact fields. Retrieval and contact-ownership validation remain release blockers. The alert/outreach runner's enrichment provider has not been switched. See [implementation and limits](docs/enrichment/openrouter-agent.md#retrieval-upgrade-2026-09-12).
