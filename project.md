...
Current default (user-selected): use `enrichWithAgent()` through one shared pipeline mapping for continuous worker, one-email runner and enrichment CLI. `BrokerEnrichment` remains an explicit legacy command. The [12-active-listing test](docs/enrichment/active-12-agent-test-2026-09-12.md) found three accepted people on two listings for 8.74¢ OpenRouter spend; it does not establish full coverage or superiority across all listings. Company-only, owner-listed and unresolved roster states must remain distinct. Generic labels and conflicting brokerages are excluded from accepted individual agents. Paid worker research requires `SCOUT_ENRICHMENT_BUDGET_USD`, a process-wide allowance that defaults to zero; durable daily budgeting remains unimplemented.
