...
The only enrichment entry point is `enrichWithAgent()`, mapped through `enrichForPipeline` for the worker, one-email runner and enrichment CLIs. At budget `$0` it uses StreetEasy + people search (`findListingAgents`) and reviewed direct adapters. Named agents are looked up at the firm, not the apartment; official profiles are opened when search snippets omit the email, and Firecrawl search covers Tavily quota failures. A positive `SCOUT_ENRICHMENT_BUDGET_USD` pays for OpenRouter discovery, then the same StreetEasy fallback if that finds no supported broker. The [12-active-listing test](docs/enrichment/active-12-agent-test-2026-09-12.md) found three accepted people on two listings for 8.74¢ OpenRouter spend; it does not establish full coverage. Company-only, owner-listed and unresolved roster states must remain distinct. Generic labels and conflicting brokerages are excluded from accepted individual agents. Durable daily budgeting remains unimplemented.
