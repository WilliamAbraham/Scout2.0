...
3. If the budget is `$0`, or paid discovery finds no supported broker, read the StreetEasy listing through Firecrawl (`findListingAgents`) and search each named person plus the distinctive firm word (`Tom Gur Voro`), not the apartment. Tavily snippets are read first; if they omit the email, official-looking profile pages (`/agents`, `/team`, the firm's own host) are opened through Firecrawl. Tavily HTTP failures (including 432) fall back to Firecrawl search. A page must name this person at this firm; a namesake at another office is refused even when the unit is absent. Agents credited to another firm are refused.
