...
`backend/src/enrichment/agent.ts` contains the improved OpenRouter agent. The continuous worker still instantiates the older `BrokerEnrichment` service. The newer agent independently identified Fatma Kara for 620 East 6th Street #9A; that run cost $0.003407284, but her personal email and phone remain unverified. One successful broker identification does not establish general recall or an outreach-ready contact.
