...
Scaffolded from the official `with-supabase` starter — see `docs/superpowers/plans/2026-08-30-nextjs-frontend.md` and `docs/superpowers/specs/2026-08-30-nextjs-frontend-design.md` for the exact constraints that shaped it (pinned dependency versions, which strict tsconfig flags must never be loosened, Tailwind v3 not v4). Auth screens (`frontend/app/auth/*`, `frontend/app/protected/`) remain alongside the public sample dashboard. The public dashboard uses in-memory demo listings. `/dashboard` implements authenticated, user-filtered read-only listing/pursuit records. Both routes expose a writable Search preferences dialog to signed-in users via `app/actions/profile.ts`; session-derived ownership and an editable-field whitelist preserve agent answers, pause and send caps. Signed-out preferences remain a sample. Live reads/writes and deployed RLS are unverified, and worker command integration remains pending. Password login lands on `/dashboard`.
