...
npm run search -w backend -- --user <id>          # score the shared listing pool for one user, enrich the matches
npm run search -w backend -- --user <id> --send   # send the opening emails (redirected; see below)
