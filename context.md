...
- **Every live send is still redirected.** `CONTROLLED_TEST_RECIPIENT` is `williamja100@gmail.com` (override with `SCOUT_TEST_RECIPIENT`), a mailbox separate from the `williamsaibroker@gmail.com` account the agent sends from, so a redirected send is visibly received rather than reading as a note to self. Outreach written to a real broker lands there instead of reaching them; `allowRealRecipients` remains the only way past it and nothing sets it.
...
Mapping rejects brokerage names and generic “Listing Agent” placeholders as people; differing-brokerage candidates cannot supply email recipients. Only complete supported rosters or eligible exact-listing team routes can supply outreach-ready snapshots. Generic office contacts remain display-only. Budget refusals stay deferred, and the one-email runner does not mark a deferred alert processed. Raw agent results and mapped pipeline results are saved separately. Verification: 233 backend and 41 frontend tests and workspace typechecks pass; root `yarn lint` remains absent.

Worker discovery follow-up on `codex/listing-broker-discovery`: `BrokerEnrichment` previously never called the listing-page reader used by the successful experimental Fatma agent. It now falls back to public listing HTML when brokerage research has no named agents. A supplied URL and one address-derived candidate are bounded by the existing host/redirect/size/time limits. Semantic Listed by cards must identify licensed people at the input brokerage; exact street/unit and rent must match. This path recovers names only and does not promote them to email recipients. `directOnly` retains its brokerage-only behavior; `listingFallback: false` isolates provider-only runs.

A fresh, direct-HTTP run using the database's original rental URL 5157949 independently returned Fatma Kara, FIND Real Estate, with profile 942807. Saved a new enrichment event for the existing #9A pursuit and refreshed the authenticated local dashboard: her name appears in the row and contact panel with the exact listing source. The missing-email blocker and null contact snapshot remain intact. No worker, paid provider or send ran. The screenshot/name was not passed as extraction input. Other already-enriched records are not automatically reprocessed.

Verification: 226 backend and 41 frontend tests pass, including default-service-to-alert persistence, wrong unit/rent/brokerage rejection, multiple listing brokers, exclusion of recommended profiles, and name retention during deferred research. Workspace typechecks and targeted frontend ESLint pass; root `yarn lint` still has no script. Full-viewport styling remains integrated.
