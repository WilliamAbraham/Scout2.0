...
`enrichWithAgent()` is now the default for the worker, one-email runner and `npm run enrich`; the older service is available as `npm run enrich:legacy`. Shared result mapping preserves named agents, brokerage contacts, evidence and cost while excluding generic labels and conflicting-brokerage recipients. Recovered names remain visible without email/phone. The [12-active-listing test](docs/enrichment/active-12-agent-test-2026-09-12.md) cost 8.74¢ in OpenRouter usage and found accepted people on only 2 listings; blocked source reads remain a major coverage gap. The worker uses `SCOUT_ENRICHMENT_BUDGET_USD` as a shared per-process allowance, default zero; configure it explicitly for paid research. Tavily/Firecrawl credits are separate.
