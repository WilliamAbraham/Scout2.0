...
`enrichWithAgent()` is the only enrichment path for the worker, one-email runner and `npm run enrich`. Shared result mapping preserves named agents, brokerage contacts, evidence and cost while excluding generic labels and conflicting-brokerage recipients. Recovered names remain visible without email/phone. At `SCOUT_ENRICHMENT_BUDGET_USD=0` the agent uses StreetEasy + people search instead of paid discovery. The [12-active-listing test](docs/enrichment/active-12-agent-test-2026-09-12.md) cost 8.74¢ in OpenRouter usage and found accepted people on only 2 listings; blocked source reads remain a major coverage gap. Tavily/Firecrawl credits are separate.
