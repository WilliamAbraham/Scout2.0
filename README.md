...
When brokerage discovery finds no named agent, the worker's enrichment service now reads the supplied StreetEasy listing (with one address-derived fallback) and extracts licensed agents from its **Listed by** section. It requires matching address, unit, rent and brokerage. Recovered names remain visible even without email/phone, including during deferred enrichment. A fresh source read and saved-record repair verified Fatma Kara for 620 East 6th Street #9A; this does not establish coverage for blocked pages or other markup.
